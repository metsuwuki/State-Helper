...
StateHelper/Arizona_bootstrap/loader.lua

Bootstrap-загрузчик

Назначение:
- создает общий runtime-context
- инициализирует bootstrap-сервисы
- запускает сборку модульного runtime

Роль в архитектуре:
- первый Lua-модуль, который грузится из `StateHelper.lua`
- связывает manifest, resolver, updater и запуск core app
...
local function sh_bootstrap_loader_get_remote_data_root()
    ...
    if release_config and type(release_config.sh_core_release_get_remote_data_root) == 'function' then
        local remote_data_root = release_config.sh_core_release_get_remote_data_root('Arizona')
        if type(remote_data_root) == 'string' and remote_data_root ~= '' then
            return remote_data_root
        end
    end
...
function loader.start(opts)
    ...
    if ctx.state.delivery.remote_ready then
        ...
        ctx.logger:warn('github delivery is not configured in Arizona_bootstrap/manifest.lua')
    end
