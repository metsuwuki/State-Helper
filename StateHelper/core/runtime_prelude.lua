...
local sh_roleplay_dir = sh_legacy_data_path('Отыгровки')
...
function autoFixFastActionsIdFlags()
        ...
        if found_cmd.arg and #found_cmd.arg > 0 then
            ...
            if first_arg then
                ...
                if desc:find('id игрока') or desc:find('ID игрока') or 
                   desc:find('айди игрока') or desc:find('id игрока') or
    ...
    if fixed_count > 0 then
        ...
        save()
        if not setting.cef_notif then
            sampAddChatMessage(string.format('[SH]{23E64A} Исправлено: %d действий перемещено.', fixed_count), 0x23E64A)
        else
            cefnotig(string.format('{23E64A}[SH] Исправлено: %d действий перемещено.', fixed_count), 3000)
        end
    end
...
function debugCommandInfo(cmd_name)
    ...
    if has_args then
        for _, arg in ipairs(found_cmd.arg) do
            table.insert(args_info, string.format("%s (%s)", arg.name, arg.type == 1 and "текст" or "число"))
        end
    end
...
function smart_punish_func(arg)
    ...
    if setting.org ~= 10 then
        if not setting.cef_notif then
            sampAddChatMessage('[SH]{FFFFFF} ' .. 'Вы не состоите в Тюрьме строгого режима.', 0xFF5345)
        else
            cefnotig('{FF5345}[SH]{FFFFFF} ' .. 'Вы не состоите в Тюрьме строгого режима.', 2000)
        end
        return
    end
...
function main()
		...
		if doesFileExist(tencode_path) then
			...
			if file then
				...
				if success and type(all_codes) == 'table' then
					...
					downloadUrlToFile(tencode_url, tencode_path, function(id, status)
						if status == 6 then loadTenCodes() end
					end)
				end
			end
		else
	...
	local old_cmds_file = sh_legacy_data_path('Отыгровки.json')
