...
StateHelper/Arizona_factions/smi/data.lua

Canonical SMI command definitions for the Arizona project.
]]
...
local function current_smi_org_id()
    local current_setting = rawget(_G, 'setting')
    local org_id = type(current_setting) == 'table' and tonumber(current_setting.org) or 16
    if org_id ~= 17 and org_id ~= 18 then
        return 16
    end
    return org_id
end

local function current_smi_station_short()
    local station_by_org = {
        [16] = 'СМИ ЛС',
        [17] = 'СМИ СФ',
        [18] = 'СМИ ЛВ',
    }
    return station_by_org[current_smi_org_id()] or 'СМИ ЛС'
end

local function current_smi_station_full()
    local station_by_org = {
        [16] = 'Радиостанция г. Лос Сантос',
        [17] = 'Радиостанция г. Сан Фиерро',
        [18] = 'Радиостанция г. Лас Вентурас',
    }
    return station_by_org[current_smi_org_id()] or 'Радиостанция г. Лос Сантос'
end

local function current_smi_station_tag()
    return '[' .. current_smi_station_short() .. ']'
end
...
function smi_data.get_command_specs()
    return {
        ...
        },
        {
            ...
            arg = {},
            act = {
                send('/d ' .. current_smi_station_tag() .. ' - [Всем]: Занимаю государственную волну, просьба не перебивать!'),
                send('/gov ' .. current_smi_station_tag() .. ': Доброго времени суток, уважаемые жители штата!'),
                send('/gov ' .. current_smi_station_tag() .. ': Сейчас проходит собеседование в организацию "' .. current_smi_station_full() .. '".'),
                send('/gov ' .. current_smi_station_tag() .. ': Для вступления нужны документы, жильё и присутствие в холле редакции.'),
                send('/d ' .. current_smi_station_tag() .. ' - [Всем]: Освобождаю государственную волну, спасибо что не перебивали.'),
            },
        },
        {
            folder = 1,
            ...
            act = {
                ...
                send('Я {myrank} {mynickrus}, представитель организации "' .. current_smi_station_full() .. '".'),
        ...
        },
        {
            ...
            arg = {},
            act = {
                send('/d ' .. current_smi_station_tag() .. ': Занимаю эфирную волну.'),
                send('/news [Интервью]: Здравствуйте, уважаемые радиослушатели! В эфире ' .. current_smi_station_full() .. '.'),
        ...
        },
        {
            ...
            act = {
                ...
                send('/d ' .. current_smi_station_tag() .. ': Освобождаю эфирную волну.'),
            },
        },
        {
            folder = 1,
            ...
            arg = {},
            act = {
                send('/d ' .. current_smi_station_tag() .. ': Занимаю новостную волну.'),
                send('/news Добрый день, дорогие радиослушатели! В эфире ' .. current_smi_station_full() .. '.'),
                ...
                send('/d ' .. current_smi_station_tag() .. ': Освобождаю новостную волну!'),
            },
        },
        {
            folder = 1,
            ...
            act = {
                send('/d ' .. current_smi_station_tag() .. ': Занимаю новостную волну.'),
                send('/news Добрый вечер, дорогие радиослушатели! В эфире ' .. current_smi_station_full() .. '.'),
        ...
        },
        {
            ...
            act = {
                send('/d ' .. current_smi_station_tag() .. ': Занимаю эфирную волну.'),
                send('/news [Викторина]: Добрый день, уважаемые радиослушатели! В эфире ' .. current_smi_station_full() .. '.'),
